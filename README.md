...
## 🎯 Установка для обычных пользователей (без программирования)
...
2. **Скачайте** `BatMon.zip` из списка файлов
3. **Распакуйте** zip-архив - двойной клик по `BatMon.zip` в папке Загрузки
4. **Найдите распакованную папку** `BatMon.app` 
5. **Запустите** `BatMon.app` (двойной клик)
6. **Terminal откроется автоматически** с интерактивным интерфейсом BatMon
7. **Готово!** 🎉

### 🛡️ Если macOS показывает предупреждение о безопасности:

**⚠️ "Apple could not verify BatMon is free of malware"** - это нормально!

**Способ 1 (простой):**
1. **НЕ нажимайте** "Move to Trash"
2. **Нажмите** "Cancel"
3. **Откройте Системные настройки** → **Конфиденциальность и безопасность**
4. **Найдите сообщение** о BatMon внизу и нажмите **"Все равно открыть"**

**Способ 2 (быстрый):**
1. **Правый клик** на `BatMon.app` → **"Открыть"**
2. **В диалоге нажмите "Открыть"**

### 🚀 Способ 2: Сборка из исходного кода (для разработчиков)

**Скопируйте эту команду в Терминал:**
...
A: База данных в `~/Library/Application Support/BatMon/batmon.sqlite`, отчеты в `~/Documents/`

**Q: Как удалить программу?**  
A: Удалите папку `batmon` и `~/Library/Application Support/BatMon/`
...
- База данных в `~/Library/Application Support/BatMon/`, отчеты в `~/Documents/`
...
После полного анализа программа покажет:

**✅ БАТАРЕЯ В ПОРЯДКЕ:**
- Износ менее 20%
- Менее 1000 циклов заряда
- Стабильная разрядка без скачков
- Температура не превышает 40°C
- **Рекомендация:** Продолжайте использовать

**⚠️ БАТАРЕЯ ТРЕБУЕТ ВНИМАНИЯ:**
- Износ 20-30%
- 1000+ циклов заряда  
- Частые аномалии разрядки
- Перегрев при нагрузке
- **Рекомендация:** Наблюдайте, планируйте замену

**🔴 БАТАРЕЯ НУЖДАЕТСЯ В ЗАМЕНЕ:**
- Износ более 30%
- Более 1500 циклов
- Резкие провалы заряда
- Система Apple рекомендует замену
- **Рекомендация:** Заменить немедленно
...
- `~/Documents/battery_report.md` - Markdown отчет с таблицами и статистикой  
- `~/Documents/battery_report.html` - HTML отчет с интерактивными графиками Chart.js
- `~/Library/Application Support/BatMon/batmon.sqlite` - База данных SQLite с историей измерений
